  ...
  return (
    <div className='min-h-[calc(100vh-56px)] w-full p-5 flex items-center justify-center'>
      <div className='w-96  bg-white rounded-md p-5 shadow border border-gray-400 '>
        ...
        {/* Custom Login Form */}
        <form onSubmit={customSignupHandler}>
          <div className='border border-gray-400 rounded-full flex items-center px-3 py-1 gap-2 overflow-hidden focus-within:border-red-500'>
            <FaRegUser />
            <input type='text' name='username' value={formData.username} onChange={inputHandler} placeholder='Username' className=' w-full h-8 outline-none'  />
          </div>
          ...
          <p className='mt-3 pl-2 text-sm'>Already have Account? <Link to="/login" className='text-blue-600  hover:text-blue-400 '>Login</Link></p>

          <button type='submit' className='bg-red-500 hover:bg-red-400 text-lg text-white text-semibold px-6 py-1 rounded mt-5 mx-auto block'>Signup</button>
        </form>
